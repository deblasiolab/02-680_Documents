...
\title{Topic 2: Sets, Operators, and Functions}
\author{02-680: Essentials of Mathematics and Statistics}
%\date{}							% Activate to display a given date or no date

\begin{document}
%% Sets first row of all tables as the header, change locally if needed
...
The second way to define a set is by abstraction:
\[
...
\end{tabular}
\end{center}
\footnotetext{While the other sets on this list are always the same size, the size of an alphabet is context dependent. Sometimes it's DNA, sometimes its capital latin (english) letters, other times it's kanji.}
...
\]
That is to say an element is the union if it's in \textit{either} of the two sets, but only in the intersection if its in both. 
...
In fact difference is so similar to subtraction that you may sometimes see the minus sign used in place of the standard backslash. 
\[
S\setminus T  := \left\{x \mid x \in S \wedge x \notin T\right\} 
\]
You can think of it as the elements that are unique to S. 
...
To describe this relationship we say one set is a \emph{subset} of another if all of its elements are in both sets. 
That is
\[
...
So if we define $C' = \{0,1,4\}$, then $C=C'$.
...
\]
In other words for $S$ to be a proper subset of $T$ it first needs to be a subset, but there needs to be at least one item in $T$ that's not in $S$.
...
(That means the power set of a power set of a set, i.e. $\mathcal{P}\left(\mathcal{P}\left(B\right)\right)$, is a set of sets of sets and an element is a set of sets....) 
...
But it is \emph{not} a function if the domain is $\reals$ and the codomain is $\ints$; 
in this case there are elements in the domain (lets say $d=1.5\in\reals$) 
for which the result of the equation part ($f(d)=3(1.5)^2+2=6.75+2=8.75$)
is not in the codomain ($8.75\notin\ints$).    
...
This is known as \emph{function composition}, defined formally as
\[
f\circ g: S \mapsto U \textnormal{ assuming } g: S \mapsto T \wedge f : T \mapsto U.
\]
In use it may look something like $f\left(g\left(x\right)\right)$, 
so $x$ would need to be from $S$, since that's the input to $f$, and since the input to $g$ needs to be from $T$, 
that also needs to be the codomain of $f$. Thus the codomain of $g$ ends up as the codomain of the composite function. 
