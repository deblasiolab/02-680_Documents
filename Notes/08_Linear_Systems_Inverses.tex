...
\title{Topic 8: Linear Systems \& Matrix Inverses}
...
Let 
\[H = \left[\begin{matrix}1 & 0 & 0\\ 0 & 1 & 0\\ \beta & 0 & 1\end{matrix}\right] \; \textnormal{where }\beta\in\reals\]
then 
\[HA = \left[\begin{matrix}1 & 0 & 0\\ 0 & 1 & 0\\ \beta & 0 & 1\end{matrix}\right]\left[\begin{matrix}1 & 4 \\ 2 & 5\\ 3 & 6\end{matrix}\right] = \left[\begin{matrix}1 & 4 \\ 2 & 5\\ 3 + 1\beta & 6+4\beta\end{matrix}\right].\]
...
The procedure above is great if $C$ is nonsingular, but what if it's not?
What if $C$ isn't even square (that is, if the system is over- or under-determined)?
...
it defines a new problem that has the same solution. 
...
\[\left[\begin{array}{cc|c} 3 & 2 & -1\\ 1 & -5 & 3\end{array}\right]\]
Once again, apply a Type III operation with $\beta=-3$ from row 2 to row 1,
\[\left[\begin{array}{cc|c} 0 & 17 & -10\\ 1 & -5 & 3\end{array}\right]\]
Apply Type II with $\alpha=\frac{1}{17}$ to row 1. 
\[\left[\begin{array}{cc|c} 0 & 1 & \frac{-10}{17}\\ 1 & -5 & 3\end{array}\right]\]
Apply Type III with $\beta=5$ from row 1 to row 2.
\[\left[\begin{array}{cc|c} 0 & 1 & \frac{-10}{17}\\ 1 & 0 & \frac{1}{17}\end{array}\right]\]
Apply Type I to swap  row 1 to row 2.
\[\left[\begin{array}{cc|c} 1 & 0 & \frac{1}{17}\\0 & 1 & \frac{-10}{17}\end{array}\right]\]
...
\paragraph{Exercise} Determine (through either method above) the solution to the following system: 
...
Type I: swap rows 1 and 3 (this puts the leading 1 in the top row):
\[\left[\begin{array}{ccccc|c} 
1 & 1 & 6 & 2 &  1 & 7\\
3 & 2 & -2 & -5 &  2 & 9\\
2 & -1 & 4 & 1 & -1 & 4\\
...
Isaak and Manougian, ``Basic Concepts of Linear Algebra''. \S 1.1-1.6\\
