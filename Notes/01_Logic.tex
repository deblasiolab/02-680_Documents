...
A \emph{compound proposition} can take into account multiple atomic propositions to create a single statement: 
\[ p \wedge q \]
...
mutual implication 	& $p \iff q$		& ``$p$ if and only if $q$'' \\
...
it's best to use parentheses to make sure your statements are clear. 
...
The text equivalent would be like saying ``if not both $p$ and $q$'' is equivalent to saying ``either not $p$ or not $q$''.
...
\begin{equation}
\left(p \vee q\right) \vee r \equiv p \vee \left(q \vee r\right) \tag{associativity}
\end{equation}
\begin{equation}
p \wedge q \equiv q \wedge p \tag{commutativity}
\end{equation}
It's important to note that negation and implication are not associative or commutative (I'll leave that as a thought experiment).
...
where $x$ is not known (or in other words it's \emph{unbound} or free, thus without a definition of $x$ we can't say if the statement is true or not. 
...
Notice that ``$x*0 = 0$'' is a proposition!
...
\paragraph{Negation (De Morgan again).}
 ...
 When negating nested qualifiers, its basically a recursive procedure: 
  \[\neg \left[\exists x : \left[ \forall y: P(x,y)\right]\right] \iff \left[\forall x : \neg\left[ \forall y: P(x,y)\right]\right] \iff \left[\forall x : \left[ \exists y: \neg P(x,y)\right]\right].\]
