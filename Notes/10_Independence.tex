...
\title{Topic 10: Linear Independence}
...
\vspace{1em}\textbf{Example.} Consider the following set of vectors in $\reals^2$:
\[\left\{\begin{matrix}
...
If $D\subseteq V$ for a vector space $V$, then $span(D)$ is always a valid subspace of $V$. 
...
the vector space defined by the set is also a subspace of $\reals^2$. 
...
\begin{enumerate}
\item $\forall b_i, b_j\in B, i\ne j: b_i \cdot b_j = 0$ (meaning all bases are orthogonal), and 
\item $\forall b \in B : \|b\|_2=1$ (all vectors are normal). 
\end{enumerate}

The nice thing about an orthonormal basis is that when you convert to coordinate representations, you preserve length and angles between vectors. 
...
But we can use a secondary basis \[B'=\left\{\left\langle\frac{1}{\sqrt{2}},\frac{1}{\sqrt{2}}\right\rangle,\left\langle\frac{1}{\sqrt{2}},\frac{-1}{\sqrt{2}}\right\rangle\right\}\] that is also an orthonormal basis for $\reals^2$. 
...
One useful quirk of this means if $T$ is an orthonormal matrix $T^{-1} = T^T$. 
Thus doing some algebra on the matrix form of the system above we see that 
we can use matrix multiplication to find the coordinate representation: $vT=\alpha$.
(I'll leave the algebra to the reader.)
\vspace{2em}
...
And for a third matrix $C\in\reals^{n\times m}$: $rank(A + C) \le rank(A) + rank(C).$

Note that in the reduced form we talked about a couple topics ago (Linear Systems), the number of non-zero rows is the rank (and the rows are going to be linearly independent). 
This also means that any nonsingular (invertible) matrix is full rank (i.e. rank is equal to $n$). 

\section*{Useful References}
Isaak and Manougian, ``Basic Concepts of Linear Algebra''. \S 3.1-3.4\\
