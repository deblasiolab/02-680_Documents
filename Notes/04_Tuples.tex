...
%%%%%
\section{Tuples}
Unlike sets, \emph{tuples} (also called \emph{sequences} or \emph{lists}) are an \textit{ordered} collection of objects.
Think of the position on a chess board (or a 2D plane), a color in RGB, etc. 
...
A very useful way to construct a \emph{set of} tuples is using the \emph{cartesian product} operator, 
...
As an example, lets remember the first two sets from our examples in the Sets topic: 
\[
...
\begin{tcolorbox}[colback=blue!5,colframe=blue!75!black,title=Quote from Liben-Nowell \S2.4]
...
\[
\{1\} \times \bigcup_{i=0}^7 \left\{0,1\right\}^i
\]
...
\]
We will define $S^0 = \{\langle\rangle\}$ (the set containing only the empty tuple) for any $S$, in the case of $\Sigma^0$ we often call it the empty string.
