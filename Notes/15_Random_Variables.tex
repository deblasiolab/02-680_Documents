...
\title{Topic 15: Random Variables}
...
The formal definition of a random variable $X$, is as a mapping: \[X: \Omega \to \reals\]
which assigns a real number $X(\omega)$ to an outcome in our space (note this is not a probability in and of itself). 

As an example, let's say we toss a coin twice, we know we get $\Omega_{twocoin}$ as defined in previous lectures. 
We can assign a variable $C(\omega)$ to be the count of the number of heads in each outcome.


We can then examine probabilities of the possible values a random variable $X$ can take on (its range), 
we usually use a lower case of the same name, $x$. 
...
As another example let's again examine rolling two dice (for simplicity, 4 sided), 
...
%\caption{Running example for $\Omega_{twocoin}$}
\begin{tabular}{c|c|c}
\textbf{$\omega$} & \textbf{$p(\omega)$} & \textbf{$V(\omega)$}\\
\hline
\texttt{11}		& 1/16 &		$2$\\
\texttt{12}		& 1/16 &		$3$\\
\texttt{13}		& 1/16 &		$4$\\
...
\paragraph{Example Scenario.} $n$ repeated coin flips, each one being independent and having probability of heads $\alpha$.
...
Poisson distributions are used to model rare events over time, with $\lambda$ representing the expected number of events per interval. 

\[p(k) = \frac{\lambda^ke^{-\lambda}}{k!} \;\; \forall k\in\Omega \text{ where } \Omega = \{0,1,2,...\}.\]
...
\[p(k) = (1-\alpha)^{k-1}\alpha \;\; \forall k\in\Omega \text{ where } \Omega = \{1,2,...\}.\]
...
We still have the same properties as before, everything integrates (sums) to 1, 
all the densities are non-negative, etc.; 
...
That's why the probability of a single value ($p(X=x)$) is (usually) zero. 
...
All outcomes with value between $a$ and $b>a$ are equally likely:
...
It is parameterized by the mean (center) value of the samples $\mu\in\reals$,
...
(notice this can also be thought of as the time between Poisson events, Sec.~\ref{subsec:poisson}).


\begin{center}
\begin{tikzpicture}[
    declare function={exp_dist(\k,\l)=(\k>=0)*\l*exp(-1*\l*\k);}
]
\begin{axis}[
...
%%%%%%%
\subsection{Discrete}
For discrete this is as follows: 
\[p(X\le x) = \sum_{y \in \text{range}(X),y\le x} p(X=y)\] %\{\omega \in \Omega \mid \omega \le x\}

In the example above, the probability 
\[p(V\le v) = \sum_{d \in \left\{x \in \ints \mid x \ge 2 \wedge x \le v\right\}}p(V=d)\]
so if we want to know
\[p(V\le 4) = \sum_{d\in\{2,3,4\}} p(V=d) = p(V=2) + p(V=3) + p(V=4) = \frac{1}{16} + \frac{1}{8} + \frac{3}{16} = \frac{3}{8}\]
...
so in the example above we can ask $p(V \le 4.78) = \frac{3}{8}$.
...
So for instance, if we look at the Gaussian distribution: 
