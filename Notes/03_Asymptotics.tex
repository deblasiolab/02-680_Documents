...
Let $p(n) = \displaystyle_{i=0}^k a_in^k$ be a polynomial of order $k$,
$p(n) \in O(n^k)$.

Basic idea of proof: for $i<k$, $a_in^i < |a_i|n^k$ for any $n>1$. 

\subsection{Logarithms \& Exponentials}
Log << Polynomial << Exponential

$log_n \in O(n^\epsilon)$ for $\epsilon >0$. (base of the log does not matter)

$p(n) \in O(b^n)$ for any $b>1$ and $k\ge 0$ for the polynomial above.

$b^n \in O(c^n) \iff b \le c$. 
