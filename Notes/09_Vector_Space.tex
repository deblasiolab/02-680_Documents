...
\item $\forall v \in V, \alpha,\beta \in \reals: (\alpha+\beta) v = \alpha v + \beta v$
\item $\forall v \in V, \alpha,\beta \in \reals: \alpha(\beta v) = (\alpha\beta) v$
...
Since multiplication is as we normally see it, we know axioms (6), 
%(8), (student correction see email, 20260923)
...
[Side note: axiom (8) also does not hold but we leave the proof of this to the reader]
...
(Kind of silly since $\reals^{m\times n}$ are matrices, not vectors but that's okay.)
...
The first dimension always remains $0$, and the second dimension inherits all of its properties from scalar addition and multiplication. 
...
\paragraph{Example.}
Lets define \[P_4 = \left\{a_4x^4 + a_3x^3 + a_2x^2 + a_1x^1 + a_0 \mid a_i\in \reals, \forall i, 0 \le i \le 4 \right\}.\]
...
Isaak and Manougian, ``Basic Concepts of Linear Algebra''. \S 2.1-2.3\\
