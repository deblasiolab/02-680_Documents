...
\title{Topic 18: Expectation and Moments}
...
\[\E[F] = \frac{1}{2}\]
The difference between $F$ and $\E[F]$ is $+\frac{1}{2}$ and $-\frac{1}{2}$ (with equal probability). 
...
\[\V[S] = \E\left[\left[S-\E\left[S\right]\right]^2\right] \]\[ = \left(0.4995 \cdot 0.5995^2\right) + \left(0.4995 \cdot 0.4005^2\right) + \left(0.001 \cdot  99.4005^2\right) 
...
If $p(S=100)=0.001$ that's approximately one in every 1000 throws,
if we throw much less than that we may think it's a fair coin. 
...
\[\V\left[X\mid Y=y\right] = \int_{-\infty}^\infty \left(\left[x-\E\left[X\mid Y=y\right]\right]^2 \cdot f(x\mid y) dx\right)\]
