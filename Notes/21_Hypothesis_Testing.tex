...
\title{Topic 21: Hypothesis Testing}
...
this is the set of events that are all outcomes other than our alternate. 
...
Errors occur when the hypothesis test tells us something that's wrong. 
So in the example above 
if the test tells us to reject the null (that is, it's confident that the cholesterol changed) 
...
In this example, once again the Type I and Type II errors are shown in the blue and red shaded regions, 
...
\begin{enumerate}
\item Compute a test statistic (a function of the data) that's appropriate for the distribution: \[T = r(X_1,X_2,\cdots,X_n),\]
\item Compute a $p$-value (we will talk about this below), then 
\item For a desired significance level $\alpha$ and the $p$-value, decide whether to retain or reject $H_0$. 
\end{enumerate}
...
Under the null hypothesis \[\overline{X_n}\sim\mathcal{N}(\mu_0, \sigma^2/n)\] 
...
\item the test statistic that's appropriate for those samples, and 
...
\begin{table}
\centering
\caption{$\chi^2$ $p$value lookup table}
\begin{tabular}
      {|r||rrr|rrr|rrr|rr|}
\hline
$\nu$&0.4&0.33&0.25&0.2&0.125&0.1&0.05&0.025&0.01&0.005
