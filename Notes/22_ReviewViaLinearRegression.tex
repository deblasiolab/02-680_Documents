...
(we covered this briefly in a previous topic (Independence), but with a slightly different formula):
\begin{itemize}
...
\hline
Expression of regulator genes & Expression of target genes\\
\hline
...
\frac{d}{d\beta}\beta^TX^TX\beta  - \frac{d}{d\beta}2 \beta^TX^Ty &= 0\\
...
\begin{align*}
\ln p(\mathcal{D}\mid \beta, \sigma) &= \ln \left[\prod_{i=1}^{n} \frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{1}{2\sigma}(y_i-x_i\beta)^2}\right]\\
	&= \sum_{i=1}^{n} \ln \left[\frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{1}{2\sigma}(y_i-x_i\beta)^2}\right]\\
...
$\hat\beta_{MLE} = (X^TX)^{-1}X^Ty$, that's just Least Squares!

But we don't know $\sigma$, that's actually, what we want to know because it models how uncertain we are with any given output.
