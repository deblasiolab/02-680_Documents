...
We say the path has \emph{length} $k-1$, and that is a path \textit{from} $v_1$ \textit{to} $v_k$.
...
A lot of times we want to talk about the \emph{neighborhood} of a node: 
...
Here $\mathds{1}: \{\texttt{true},\texttt{false}\}\mapsto\{1,0\}$ in an indicator function that produces the value 1 when the input is true, and 0 otherwise.  
...
and thus $N_G(v) = N^{in}_G(v) \cup N^{out}_G(v)$ (and the degrees are the respective set cardinalities). 
...
a cycle is a special type of path $\langle v_1, v_2,.... v_{k}, v_1\rangle$ ($k\ge 2$) that ends at the same node it started at. 
...
A $k$-ary tree is one where every internal node has degree at most $k+1$; that is 
$\forall v\in V, v $ is internal $: |N(v)| \le k+1$. 
