...
\title{Topic 19: Parameter Estimation and Maximum Likelihood Estimation}
...
\[\lim_{n\rightarrow\infty} p\left(\overline{X_n}=\mu\right) \to 1.\]

\begin{aside}[pedantry detail]
Technically the probability that a continuous random variable's sample mean 
is exactly equal to $\mu$ is generally 0 for any finite $n$, and stays 0 in the limit.
Thus it's more precise to say that it's within some small region around $\mu$. 
\[\lim_{n\rightarrow\infty} p(|\overline{X_n}-\mu| < \epsilon) \to 1, \text{ for any } \epsilon > 0\]
\end{aside}
...
the distribution of $\overline{X_n}$ converges to the Gaussian (normal) distribution $N\left(\mu, \frac{\sigma^2}{n}\right)$. 
...
We will use \emph{Statistical Inference} or \emph{Learning} to try to make the models we've been learning about
...
Statistical inference is the task of trying to learn the parameters from the observations. 
...
\[\mathcal{F}\coloneq\{f(x \mid  \theta):\theta\in\Theta\},\]
where 
...
This also takes the product to a sum. 
...
(but we also need to ensure it's a max and not a min or saddle.)
If there is no critical point, then we will use the maximum allowable parameter value. 

\paragraph{Example. }
Let's look at flipping a coin $n$ times, but we don't know if it's fair or not. 
So each of the $\mathcal{D} = X_1, X_2, \cdots, X_n$ trials are independent, 
thus $X_i \sim Bernoulli(\alpha)$, and thus $f(X_i=x\mid\theta)=\alpha^x(1-\alpha)^{1-x}$, 
here $\theta = \{\alpha\}$. 
...
where $X_i \sim Bernoulli(\alpha)$.
Find the MLE of $\theta=\{\alpha\}$.

\begin{align*}
\frac{d}{d\theta} \ln p(\mathcal{D}\mid\theta) 	&= \frac{d}{d\theta} \ln \prod_{i=1}^n p(X_i\mid\theta)\\
...
Therefore, for a set of $n$ Bernoulli random variables with $s$ successes, (when we solve the previous equation equal to 0) 
\[\hat\alpha = \frac{s}{n}\]
...
\frac{d}{d\sigma} -n \ln \sigma - \frac{1}{2\sigma^2}\sum_{i=1}^n (x-\mu)^2&= 0\\
...
\begin{align*}
\ln p(\mathcal{D}\mid\lambda) & = \ln\left(\prod_{i=1}^n p(X_i\mid\lambda)\right)\\
